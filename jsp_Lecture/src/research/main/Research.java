...
public class Research
{
	...
	public Research(String pTitle, String pCustomer, String pSubject, int pQuestionNumber, int pMaxAnswerNumber, String pOpendate, String pClosedate)
	{
		title = pTitle;
		customer = pCustomer;
		subject = pSubject;
		max_qnum = pQuestionNumber;
		max_anum = pMaxAnswerNumber;
		opendate = pOpendate;
	...
	public int getMaxAnswerNumber() {
		int maxAnswerNumber = 0;
		for (int i = 0; i < listQA.size(); i++) {
			List<String> answer = listQA.get(i).getAnswer();
			if (i > 0) {
				if (maxAnswerNumber < answer.size()) {
					maxAnswerNumber = answer.size();
				}
			} else if (i == 0) {
				maxAnswerNumber = answer.size();
			}
		}
		return maxAnswerNumber;
	}
